...
4. 예: `재생에너지 보고서를 찾아 슬라이드에 넣어 줘`
5. 인터넷은 주소창으로 실사이트를 엽니다. 문서·시트·슬라이드는 화면 안 타일입니다.
6. `install os.neu` 또는 상단 **다운로드** 로 Linux에 심으면 앱 목록·바탕화면에 **os.neu** 아이콘이 생깁니다. 폴더로 이동하지 않아도 됩니다. 창 모서리를 끌거나 상단 창 아이콘으로 크기를 조절합니다.

## 9. 매뉴얼 다시 보기

화면 **위쪽 Manual** 아이콘 → **os.neu manual**. 맨 아래 **docs.neuavenue.com** 이 이 문서 카드입니다. 카드의 × 는 매뉴얼로, 매뉴얼의 × 는 닫기입니다. **다운로드** 아이콘은 Linux 스크린 팩(`neuos-linux.tar.gz`)입니다.

회사 열쇠를 이미지에 심지 않습니다. 공개 AI는 이 PC에서 받을 수 있습니다. 확인 없는 삭제·설치는 하지 않습니다.

## 10. 나가기

그래픽 랜딩·설치·말하기 홈에서 오른쪽 위 **× 는 종료**입니다. Ubuntu 바탕화면으로 돌아갑니다. 안쪽 화면의 × 는 이전 화면입니다.
